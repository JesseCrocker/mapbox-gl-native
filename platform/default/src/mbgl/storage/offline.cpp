...
OfflineTileListRegionDefinition::OfflineTileListRegionDefinition(std::string styleURL_, LatLngBounds bounds_, double minZoom_, double maxZoom_, float pixelRatio_, bool includeIdeographs_, std::vector<CanonicalTileID> tileList_)
    : styleURL(std::move(styleURL_)),
...
OfflineRegionDefinition decodeOfflineRegionDefinition(const std::string& region) {
    ...
    if (doc.HasMember("tile_list")) {
        ...
        return OfflineTileListRegionDefinition{
            ...
            minZoom, maxZoom, pixelRatio, includeIdeographs, tileList};
